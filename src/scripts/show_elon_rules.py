...
def show_elon_rules():
    ...
    # Check simple sect calculation if summary is missing
    # But usually it's in a summary dict or top level?
    # Actually auditor.perform_audit returns NO summary key!
    # It calculates sect internally but doesn't return it explicitly except perhaps in 'dignity' or 'planets_forensic'?
    # Let's check chart.sun_altitude from the raw data if needed, but let's look for it in analysis.
    
    sect = "Unknown"
    # Try find sect in 'dignity' or 'advanced_mechanics' or 'summary'
    # Actually perform_audit sets `sect = ...` but doesn't store it in `analysis` directly?
    # Wait, check code again.
