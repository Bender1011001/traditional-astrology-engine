...
from src.engine.horary import (CONDITION_WEIGHTS, NEGATIVE_CONDITIONS,
                               POSITIVE_CONDITIONS, analyze_horary_physics,
                               build_horary_oracle, calculate_antiscia,
                               check_mutual_reception, check_refranation,
                               evaluate_horary_conditions, get_aspect_distance,
                               get_house_sign, get_moiety_orb,
                               get_nature_score,
                               get_sect_score, is_applying,
                               score_conditions, select_quesited_house)
...
def test_refranation_slow_planet():
    ...
    p1 = Planet(
        name=PlanetName.MERCURY, longitude=85.0, speed=0.01
    )  # Nearly stationary
    p2 = Planet(name=PlanetName.JUPITER, longitude=90.0, speed=0.08)
