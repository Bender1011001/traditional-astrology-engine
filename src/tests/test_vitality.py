...
def create_vitality_chart(sun_lon, moon_lon, asc_lon, sun_alt):
    ...
    moon = Planet(name=PlanetName.MOON, longitude=moon_lon, altitude=1.0) # Assume moon is up
    
    # Needs a ruler to aspect it. Let's add Mars in Aries (Domicile) to aspect Sun in Aries.
    mars = Planet(name=PlanetName.MARS, longitude=15.0) # Conjunct or close to Sun at 10
