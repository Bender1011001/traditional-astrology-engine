...
def test_astrology_tools_forensic_audit():
    ...
    with patch("src.astrology_tools.Auditor.generate_full_nativity") as mock_audit:
        ...
        mock_audit.assert_called_once_with(birth_dt=datetime(1996, 8, 13, 7, 18), city="Fairfield", state="CA")
...
def test_astrology_tools_find_electional_window():
    ...
    result = tools.find_electional_window("Fairfield", "CA", "mercantile", 48, "2026-03-22T00:00:00")
...
def test_astrology_tools_format_electional_report():
    tools = AstrologyTools()
    windows_data = {
        "query": {"location": "Fairfield, CA", "activity": "general", "scan_range": "48 hours"},
        "best_windows": [
            {
    ...
    report_with_natal = tools.format_electional_report(
        windows_data,
        natal_context={"almuten": "Sun", "time_lord": "Moon", "recommendations": ["Do something"]}
    )
