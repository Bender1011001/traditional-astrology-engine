...
from src.engine.lots import calculate_lot, calculate_all_lots, calculate_lot_position
...
def test_fortune_spirit_sect_reversal():
    ...
    assert abs(day_lots[LotName.FORTUNE.value] - night_lots[LotName.SPIRIT.value]) < 0.01
    assert abs(day_lots[LotName.SPIRIT.value] - night_lots[LotName.FORTUNE.value]) < 0.01
...
def test_hermetic_seven_present():
    ...
    chart = _make_chart()
    lots = calculate_all_lots(chart, Sect.DAY)
    for lot_name in [LotName.FORTUNE, LotName.SPIRIT, LotName.NECESSITY,
                     LotName.EROS, LotName.COURAGE, LotName.VICTORY, LotName.NEMESIS]:
