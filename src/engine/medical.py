...
class MedicalAstrology:
    ...
    def can_perform_surgery(
        ...
        if m_mars:
            afflictions.append(m_mars)

        m_saturn = check_aspect(moon_lon, saturn_lon, "Saturn")
        if m_saturn:
            afflictions.append(m_saturn)
        ...
        if target_sign and moon_sign == target_sign:
            safe = False
            reasons.append(
                f"Moon is in {moon_sign.value}, which rules {MedicalAstrology.MELOTHESIA[target_sign]}."
            )
        ...
        for ec in eclipses:
            if abs(ec["jd"] - jd_current) <= 3:
                safe = False
                reasons.append(
                    f"Surgery near {ec['type']} (JD {ec['jd']:.2f}). Avoid ±3 days."
                )
    ...
    @staticmethod
    def check_moon_mercury_interference(
    ...
    @staticmethod
    def calculate_remediation_window(
        jd_start: float, duration_days: int = 30
    ) -> List[Dict]:
        """
