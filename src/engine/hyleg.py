...
class HylegAlcocodenEngine:
    ...
    def determine_hyleg(chart: Chart) -> Dict:
        ...
        """
        from .lots import LotName, calculate_lot_position
        ...
        for name in candidates:
            ...
            if HylegAlcocodenEngine._is_in_hylegical_house(planet, chart):
                if HylegAlcocodenEngine._has_aspect_from_ruler(
                    planet.longitude, chart, sect
                ):
                    return {
                        "type": "Planet",
                        "name": name.value,
        ...
        try:
            ...
            lot_house = DignityCalculator.get_house_number(
                lot_fortune, chart.ascendant, getattr(chart, "houses", None)
            )
            if lot_house in HylegAlcocodenEngine.HYLEGICAL_HOUSES:
                if HylegAlcocodenEngine._has_aspect_from_ruler(
                    lot_fortune, chart, sect
                ):
                    return {
                        "type": "Lot",
                        "name": "Fortune",
            ...
            logger.warning(
                "Lot of Fortune Hyleg check failed: %s", repr(e), exc_info=True
            )
    ...
    @staticmethod
    def determine_alcocoden(
        hyleg_data: Dict, chart: Chart, method: str = "bonatti_points"
    ) -> Dict:
        """
        ...
        for pn, score in points.items():
            ...
            if not p_obj:
                continue
            candidates.append(
                {"name": pn, "score": score, "planet": p_obj, "via": method}
            )
        ...
        def _degree_aspects(
            p_lon: float, target_lon: float, orb: float = 12.0
        ) -> Optional[str]:
        ...
        def _sign_aspects(p_lon: float, target_lon: float) -> Optional[str]:
            ...
            diff = (ps - ts) % 12
            mapping = {
                0: "Conjunction (Whole Sign)",
                2: "Sextile (Whole Sign)",
                3: "Square (Whole Sign)",
                4: "Trine (Whole Sign)",
                6: "Opposition (Whole Sign)",
                8: "Trine (Whole Sign)",
                9: "Square (Whole Sign)",
                10: "Sextile (Whole Sign)",
            }
        ...
        orb_hits = []
        for c in candidates:
            asp = _degree_aspects(c["planet"].longitude, h_lon)  # type: ignore
            if asp:
                c2 = dict(c)
                c2["aspect"] = asp
        ...
        sign_hits = []
        for c in candidates:
            asp = _sign_aspects(c["planet"].longitude, h_lon)  # type: ignore
            if asp:
                c2 = dict(c)
                c2["aspect"] = asp
    ...
    def calculate_lifespan(hyleg: Dict, alcocoden: Dict, chart: Chart) -> Dict:
        ...
        house = DignityCalculator.get_house_number(
            p_obj.longitude, chart.ascendant, getattr(chart, "houses", None)
        )
        dignity = DignityCalculator.calculate_planet_dignity(
            p_name, p_obj.longitude, Sect.DAY if chart.sun_altitude > 0 else Sect.NIGHT
        )
        ...
        for p in chart.planets:
            ...
            if aspect_type:
                ...
                if mod != 0:
                    ...
                    logs.append(
                        f"{action} {abs(mod):.1f} ({p.name.value} {aspect_type})"
                    )
        ...
        if total >= 70:
            ...
            rating = "Cautionary (Requires strengthening)"
    ...
    def determine_anareta(hyleg_data: Dict, chart: Chart) -> Dict:
            ...
            for asp, label, orb_allow in HARD_ASPECTS:
                ...
                if orb <= orb_allow:
                    ...
                    if beats_current:
                        ...
                        best_source = (
                            "malefic"
                            if pn in [PlanetName.MARS, PlanetName.SATURN]
                            else "8th_ruler"
                        )
        ...
        sect_note = (
            " (Out-of-sect malefic — primary troublemaker)"
            if best_is_oos
            else " (In-sect malefic)"
        )
        source_note = ""
