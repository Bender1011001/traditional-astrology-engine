...
from .calculations import (calculate_solar_status, is_besieged,
                           is_void_of_course)
...
class ElectionalEngine:
    ...
    def _calculate_lightweight_chart(
        ...
        try:
            ...
            logger.warning(
                "House calc failed (extreme lat?): %s", repr(e), exc_info=True
            )
    ...
    def evaluate_chart(self, chart: Chart, activity: str) -> Dict:
        ...
        if is_besieged(moon, chart):
            score -= 40
            details.append(
                "Moon is Besieged (Trapped between Mars/Saturn) - Critical Weakness"
            )
            is_viable = False
        ...
        if 195.0 <= moon.longitude <= 225.0:
            score -= 30
            details.append(
                "Moon in Via Combusta (Burning Way, 15° Lib – 15° Sco) - Critical Weakness"
            )
            is_viable = False
        ...
        if asc_deg_in_sign < 3.0:
            score -= 20
            details.append(
                f"Ascendant at {asc_deg_in_sign:.1f}° (Bonatti #6: Too early to judge)"
            )
        ...
        if asc_deg_in_sign >= 27.0:
            score -= 25
            details.append(
                f"Ascendant at {asc_deg_in_sign:.1f}° (Bonatti #7/#30: Matter is past, don't begin)"
            )
            is_viable = False
        ...
        if asc_ruler:
            ...
            dignity = DignityCalculator.calculate_planet_dignity(
                asc_ruler.name, asc_ruler.longitude, sect
            )
            r_score = dignity["total_score"]
            score += r_score * 3  # Heavily weighted
            details.append(
                f"Ascendant Ruler ({asc_ruler_name.value}) in {asc_sign.value} has Essential Dignity of {r_score} (x3)"
            )
            ...
            if asc_ruler.name != PlanetName.SUN:
                ...
                if ruler_solar == "COMBUST":
                    ...
                    score += 30
                    details.append(
                        f"Ascendant Ruler is CAZIMI (In the Heart of Sun) - Supreme Power"
                    )
        ...
        for p in chart.planets:
            ...
            if p.name in [PlanetName.JUPITER, PlanetName.VENUS]:
                if house_num == 1:
                    score += 20
                    details.append(
                        f"Benefic {p.name.value} in 1st House - Strong Fortification"
                    )
                if house_num == 10:
                    score += 15
                    details.append(
                        f"Benefic {p.name.value} in 10th House (MC) - Success/Visibility"
                    )
            ...
            if p.name in [PlanetName.MARS, PlanetName.SATURN]:
                ...
                if house_num == 1:
                    ...
                    score -= penalty
                    details.append(
                        f"Malefic {p.name.value} in 1st House - {'Severe ' if is_oosect else ''}Obstruction"
                    )
                if house_num == 10:
                    penalty = 20 if is_oosect else 10
                    score -= penalty
                    details.append(
                        f"Malefic {p.name.value} in 10th House - {'Severe ' if is_oosect else ''}Reputational Risk"
                    )
        ...
        if activity.lower() in ["contract", "signing", "mercantile"]:
            # Focus on Mercury
            mercury = next(
                (p for p in chart.planets if p.name == PlanetName.MERCURY), None
            )
            if mercury:
                m_dig = DignityCalculator.calculate_planet_dignity(
                    mercury.name, mercury.longitude, sect
                )
                score += m_dig["total_score"] * 2
                details.append(
                    f"Activity Sig (Mercury) Dignity: {m_dig['total_score']} (x2)"
                )
                if mercury.speed < 0:
                    score -= 20
            ...
            if venus:
                v_dig = DignityCalculator.calculate_planet_dignity(
                    venus.name, venus.longitude, sect
                )
                score += v_dig["total_score"] * 2
                details.append(
                    f"Activity Sig (Venus) Dignity: {v_dig['total_score']} (x2)"
                )
            ...
            if mars:
                ...
                m_dig = DignityCalculator.calculate_planet_dignity(
                    mars.name, mars.longitude, sect
                )
                score += (m_dig["total_score"] + target_score) * 2
                details.append(
                    f"Activity Sig (Mars) Strength: {m_dig['total_score'] + target_score} (x2)"
                )
