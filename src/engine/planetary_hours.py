...
class PlanetaryHourEngine:
    ...
    def _calculate_sun_times(lat: float, lon: float, date: datetime) -> Dict:
        ...
        res_rise = swe.rise_trans(
            jd_midnight, swe.SUN, calc_rise, geopos, 0, 0, ephe_flags
        )
        ...
        res_next_rise = swe.rise_trans(
            jd_noon, swe.SUN, calc_rise, geopos, 0, 0, ephe_flags
        )
        ...
        if len(res_next_rise) > 0:
            next_rise_jd = res_next_rise[1][0]
        else:
            next_rise_jd = 0

        return {"rise_jd": rise_jd, "set_jd": set_jd, "next_rise_jd": next_rise_jd}
    ...
    def calculate_hours(chart_dt: datetime, lat: float, lon: float) -> Dict:
        ...
        return {
            ...
            "next_hour_ruler": PlanetaryHourEngine.CHALDEAN_ORDER[
                (current_planet_idx + 1) % 7
            ].value,
        }
