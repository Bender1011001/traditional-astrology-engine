...
from .reference_data import (DOMICILES, DOROTHEAN_TRIPLICITY, EGYPTIAN_TERMS,
                             EXALTATIONS, FACES_ORDER, PTOLEMAIC_TERMS,
...
class Reception:
    ...
    is_operative: (
        bool  # True if there is an applying aspect (or any aspect in Standard mode?)
    )
    mode: str
    mitigation: str = "None"
...
class ReceptionEngine:
    ...
    @staticmethod
    def _get_triplicity_rulers(
        element: str, sect: Sect, mode: ReceptionMode
    ) -> List[PlanetName]:
    ...
    @staticmethod
    def _get_term_ruler(
        sign: Sign, degree: float, mode: ReceptionMode
    ) -> Optional[PlanetName]:
        try:
            terms = (
                EGYPTIAN_TERMS[sign]
                if mode == ReceptionMode.STRICT_BONATTI
                else PTOLEMAIC_TERMS[sign]
            )
        except KeyError:
    ...
    def analyze_reception(
        cls, guest: Planet, host: Planet, chart: Chart, mode: ReceptionMode
    ) -> Reception:
        """
        ...
        rel_aspect = next(
            (
                ...
                if (a.planet_a == guest.name and a.planet_b == host.name)
                or (a.planet_a == host.name and a.planet_b == guest.name)
            ),
            None,
        )
        ...
        if rel_aspect and is_valid:
            ...
            if mode == ReceptionMode.STRICT_BONATTI:
                ...
                is_operative = (
                    True  # Standard allows separating but decreasingly effective
                )
    ...
    @classmethod
    def calculate_mutual_receptions(
        cls, chart: Chart, mode: ReceptionMode = ReceptionMode.STANDARD_LILLY
            ...
            for j in range(i + 1, len(planets)):
                ...
                if rec1.is_valid and rec2.is_valid:
                    ...
                    if "Domicile" in rec1.dignities and "Domicile" in rec2.dignities:
                        ...
                        "Exaltation" in rec1.dignities
                        and "Exaltation" in rec2.dignities
                    ):
                        r_type = "Pure Exaltation"
                    elif (
                        "Domicile" in rec1.dignities and "Exaltation" in rec2.dignities
                    ) or (
