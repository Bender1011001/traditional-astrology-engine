...
class SynastryEngine:
    ...
    def analyze_structural_fit(self, person_a: Chart, person_b: Chart) -> Dict[str, Any]:
        ...
        for planet in person_a.planets:
            if self.check_conjunction(planet.longitude, fortune_b):
                delineation = self._get_planet_on_fortune_delineation("Person A", planet.name, "Person B")
                dependencies.append({
        ...
        for planet in person_b.planets:
            if self.check_conjunction(planet.longitude, fortune_a):
                delineation = self._get_planet_on_fortune_delineation("Person B", planet.name, "Person A")
                dependencies.append({
                    "type": "Planet on Fortune",
                    "subject": "Person B",
                    "planet": planet.name.value,
        ...
        return {
            ...
            "overall_assessment": "Structural Fit" if (len(dependencies) > 0 or len(shared_fate) > 0) else "Individual Journeys"
        }

    def _get_planet_on_fortune_delineation(self, subj: str, planet: PlanetName, target: str) -> str:
        delineations = {
        ...
        return delineations.get(planet, f"{subj}'s {planet.value} impacts {target}'s Lot of Fortune.")
