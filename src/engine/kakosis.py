...
class KakosisEngine:
    ...
    def is_malefic_for_sect(planet_name: PlanetName, sect: Sect) -> bool:
        ...
        if sect == Sect.DAY and planet_name == PlanetName.MARS:
            return True
        if sect == Sect.NIGHT and planet_name == PlanetName.SATURN:
            return True
        return True  # Still technically a malefic, just 'mitigated' if in sect.

    @staticmethod
    def check_maltreatments(
        planet: Planet, chart: Chart
    ) -> List[MaltreatmentCondition]:
        conditions = []
    ...
    @staticmethod
    def _check_overcoming(
        planet: Planet, chart: Chart, sect: Sect
    ) -> List[MaltreatmentCondition]:
        """
        ...
        for potential_malefic in chart.planets:
            ...
            if m_idx == tenth_idx:
                ...
                res.append(
                    MaltreatmentCondition(
                        "Overcoming", potential_malefic.name, msg, sev
                    )
                )
    ...
    def _check_opposition(
        ...
        for m in chart.planets:
            if m.name not in KakosisEngine.MALEFICS:
                continue
    ...
    def _check_adherence(
        ...
        for m in chart.planets:
            ...
            if diff < 6:  # Orb frame
                ...
                is_applying = False
                if d_lon > 0 and rel_speed > 0:
                    is_applying = True  # P chasing M
                if d_lon < 0 and rel_speed < 0:
                    is_applying = True  # P Rx into M or M chasing P?
    ...
    def _apply_intervention(
        planet: Planet, chart: Chart, conditions: List[MaltreatmentCondition]
    ) -> List[MaltreatmentCondition]:
        """
        ...
        for cond in conditions:
            ...
            if best_mitigation > 0:
                ...
                cond.description += (
                    f" (Mitigated by Benefic Intervention: -{best_mitigation} severity)"
                )
                if cond.severity < 1:
                    cond.severity = 1
