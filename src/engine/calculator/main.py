    ...
    def calculate_chart(
        ...
        for name, pid in planets_data.items():
            ...
            try:
                ...
                if name == "North_Node": enum_name = PlanetName.NORTH_NODE
                else: continue
            ...
            if enum_name in [PlanetName.MERCURY, PlanetName.VENUS, PlanetName.MARS, PlanetName.JUPITER, PlanetName.SATURN]:
                 is_ori = PhasisEngine.is_oriental(long_val, sun_lon)
                 prox_enum = PhasisEngine.get_solar_proximity(long_val, sun_lon)
                 is_vis = PhasisEngine.calculate_visibility(jd, lat, lon, enum_name, long_val, lat_val, sun_lon)
        ...
        if "North_Node" in planets_data:
            # Find NN object
            nn_obj = next((p for p in chart_planets if p.name == PlanetName.NORTH_NODE), None)
            if nn_obj:
...
def calculate_chart_data(
    ...
    for p in chart.planets:
        ...
        if p.name == PlanetName.NORTH_NODE: key_name = "North_Node"
        if p.name == PlanetName.SOUTH_NODE: key_name = "South_Node"
        
        results["planets"][key_name] = {
    ...
    for p in chart.planets:
        ...
        if key_name not in results["planets"]: continue
        ...
        if "classical" not in results["planets"][key_name]:
             results["planets"][key_name]["classical"] = {}
             
        results["planets"][key_name]["classical"]["antiscia"] = {
             "longitude": antiscia.antiscia_lon,
             "sign": antiscia.antiscia_sign.value
        }
        results["planets"][key_name]["classical"]["contra_antiscia"] = {
             "longitude": antiscia.contra_antiscia_lon,
             "sign": antiscia.contra_antiscia_sign.value
        }
        results["planets"][key_name]["classical"]["dodecatemorion"] = {
        ...
        if p.name not in [PlanetName.URANUS, PlanetName.NEPTUNE, PlanetName.PLUTO, PlanetName.NORTH_NODE, PlanetName.SOUTH_NODE]:
             results["planets"][key_name]["classical"]["phasis"] = {
                 "phase": p.phase.value if p.phase else None,
                 "solar_proximity": p.solar_proximity.value if p.solar_proximity else None,
    ...
    try:
        ...
        if p_hours:
             results["classical"]["planetary_hours"] = {
             ...
             if p_hours.day_lord: day_lord_enum = PlanetName(p_hours.day_lord)
             if p_hours.hour_lord: hour_lord_enum = PlanetName(p_hours.hour_lord)

        almuten_data = AlmutenEngine.calculate_almuten(chart, day_lord_enum, hour_lord_enum)
    ...
    for d in doryphory_list:
        ...
        if p_key in results["planets"]:
             if "doryphory" not in results["planets"][p_key]["classical"]:
                 results["planets"][p_key]["classical"]["doryphory"] = []
             results["planets"][p_key]["classical"]["doryphory"].append({
                 "type": d.type,
    ...
    if sun_p and moon_p:
        ...
        for p in chart.planets:
             if p.name == PlanetName.SOUTH_NODE: continue
             key_name = p.name.value.title()
             if p.name == PlanetName.NORTH_NODE: key_name = "North_Node"
             
             z_ruler = MonomoiriaEngine.get_zoidion_monomoiria(p.longitude)
             t_ruler = MonomoiriaEngine.get_trigonal_monomoiria(p.longitude, is_day, sun_sign, moon_sign)
             
             if key_name in results["planets"]:
                  results["planets"][key_name]["classical"]["monomoiria"] = {
    ...
    for p in chart.planets:
        ...
        if p.name in dodeca_valens:
            if key_name in results["planets"]:
                results["planets"][key_name]["classical"]["dodecatemoria"] = {
                    "valens": dodeca_valens[p.name],
                    "paul": dodeca_paul[p.name]
                }
    ...
    for p in chart.planets:
         ...
         if maltreatments:
              ...
              if p.name == PlanetName.NORTH_NODE: key_name = "North_Node"
              if key_name in results["planets"]:
                  results["planets"][key_name]["classical"]["kakosis"] = [
                      {"condition": c.type, "malefic": c.malefic.value if c.malefic else "None", "details": c.description} for c in maltreatments
                  ]

    # 8. Receptions
    strict = ReceptionEngine.calculate_mutual_receptions(chart, ReceptionMode.STRICT_BONATTI)
    std = ReceptionEngine.calculate_mutual_receptions(chart, ReceptionMode.STANDARD_LILLY)
    results["classical"]["receptions"] = {
        "strict_bonatti": [{"planets": [m.planet_a.value, m.planet_b.value], "type": m.type, "score": m.strength_score} for m in strict],
        "standard_lilly": [{"planets": [m.planet_a.value, m.planet_b.value], "type": m.type, "score": m.strength_score} for m in std]
    }

    # 9. Aspects
    all_aspects = AspectEngine.calculate_aspects(chart)
    results["aspects"] = [
        {"p1": a.planet_a.value, "p2": a.planet_b.value, "type": a.type.value, "orb": round(a.orb, 2), "is_applying": a.is_applying, "text": a.text}
    ...
    if compare_house_systems:
        # Re-calc house comparison
         ...
         for c in COMPARE_SYSTEMS:
            if c not in systems: systems.append(c)
         if house_code not in systems: systems.insert(0, house_code)
