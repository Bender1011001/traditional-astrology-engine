...
def check_mutual_reception(p1: Planet, p2: Planet, chart: Chart) -> Optional[Dict]:
    ...
    # Types of reception
    reception_1_to_2 = []
    if p1_pos_rulers.get("domicile") == p2.name: reception_1_to_2.append("Domicile")
    if p1_pos_rulers.get("exaltation") == p2.name: reception_1_to_2.append("Exaltation")
    if p1_pos_rulers.get("triplicity") == p2.name: reception_1_to_2.append("Triplicity")
    if p1_pos_rulers.get("term") == p2.name: reception_1_to_2.append("Term")
    if p1_pos_rulers.get("face") == p2.name: reception_1_to_2.append("Face")
    
    reception_2_to_1 = []
    if p2_pos_rulers.get("domicile") == p1.name: reception_2_to_1.append("Domicile")
    if p2_pos_rulers.get("exaltation") == p1.name: reception_2_to_1.append("Exaltation")
    if p2_pos_rulers.get("triplicity") == p1.name: reception_2_to_1.append("Triplicity")
    if p2_pos_rulers.get("term") == p1.name: reception_2_to_1.append("Term")
    if p2_pos_rulers.get("face") == p1.name: reception_2_to_1.append("Face")
    ...
    if reception_1_to_2 and reception_2_to_1:
         ...
         is_major = any(r in major for r in reception_1_to_2) and any(r in major for r in reception_2_to_1)
...
def check_void_of_course_hellenistic(moon: Planet, chart: Chart) -> bool:
    ...
    for planet in chart.planets:
        if planet.name == PlanetName.MOON or planet.name in {PlanetName.NORTH_NODE, PlanetName.SOUTH_NODE, PlanetName.URANUS, PlanetName.NEPTUNE, PlanetName.PLUTO}:
            continue
...
def check_strictures(chart: Chart) -> List[str]:
    ...
    if asc_deg < 3.0:
        strictures.append("Ascendant in early degrees (< 3°): The matter is premature or not fully developed.")
    elif asc_deg > 27.0:
        strictures.append("Ascendant in late degrees (> 27°): The matter is already settled and too late to change.")
    ...
    if saturn:
        ...
        if sat_sign_idx == asc_sign_idx and asc_ruler != PlanetName.SATURN:
            strictures.append("Saturn in 1st House/Sign: The question is damaged or the querent is structurally restricted.")
        elif sat_sign_idx == desc_sign_idx and desc_ruler != PlanetName.SATURN:
            strictures.append("Saturn in 7th House/Sign: The astrologer's judgment may be impaired, delayed, or blocked.")
            
    if mars:
        mars_sign_idx = int(mars.longitude / 30) % 12
        if mars_sign_idx == desc_sign_idx and desc_ruler != PlanetName.MARS:
             strictures.append("Mars in 7th House/Sign: The astrologer may be overly aggressive, rash, or mathematically blinded.")
    ...
    if moon:
        ...
        alleviated = moon_sign in {Sign.TAURUS, Sign.CANCER, Sign.SAGITTARIUS, Sign.PISCES}
        
        if (voc_lilly or voc_hellenistic) and alleviated:
            strictures.append(f"Moon is Void of Course, but mitigated by being in {moon_sign.value} (Taurus, Cancer, Sagittarius, or Pisces). The matter may still proceed.")
        else:
            if voc_lilly and not voc_hellenistic:
                strictures.append("Moon is Void of Course (Medieval/Lilly Out-of-Sign Rule): The Moon makes no aspect before leaving its sign. The matter goes hardly on.")
            elif voc_lilly and voc_hellenistic:
                strictures.append("Moon is strictly Void of Course (Both Hellenistic & Medieval): Absolutely nothing will come of the matter.")
    ...
    if moon and 195.0 <= moon.longitude <= 225.0:
        if 202.5 <= moon.longitude <= 204.5: # 2 degree orb for Spica
            strictures.append("Moon in the Via Combusta, but Conjunct Spica: The Querent is walking through fire, but is supremely protected by unexpected grace.")
        else:
            strictures.append("Moon in the Via Combusta (15° Libra to 15° Scorpio): The querent is highly distressed; sudden shifts or destructive energies surround the matter.")
    ...
    if moon and sun:
        ...
        if dist > 180:
            dist = 360 - dist
        if (17/60) < dist <= 8.5:
            strictures.append("Moon is Combust (-Cazimi): The querent or the matter is overwhelmed, blinded, or severely weakened by a superior force.")
    ...
    if sun and asc_planet and asc_ruler != PlanetName.SUN:
        ...
        if dist > 180:
            dist = 360 - dist
        if (17/60) < dist <= 8.5:
            strictures.append(f"Ascendant Ruler ({asc_ruler.value}) is Combust: The Querent is incapable of action, imprisoned, or structurally destroyed.")
            
    # 7. Ascendant Ruler is Retrograde
    if asc_planet and asc_planet.speed < 0 and asc_planet.name not in {PlanetName.SUN, PlanetName.MOON}:
        strictures.append(f"Ascendant Ruler ({asc_ruler.value}) is Retrograde: The Querent will likely change their mind or withdraw from the matter.")
        
    # 8. Moon or Ascendant Ruler Conjunct the South Node (Cauda Draconis)
    south_node = next((p for p in chart.planets if p.name == PlanetName.SOUTH_NODE), None)
    if south_node:
    ...
    if moon and (moon.longitude % 30) >= 27.0:
        strictures.append("Moon in Late Degrees (>= 27°): The current alignment is rapidly dissolving; the Querent is frantic or the situation is highly unstable.")
    ...
    if desc_planet:
        if desc_planet.speed < 0 and desc_ruler not in {PlanetName.SUN, PlanetName.MOON}:
            strictures.append(f"7th House Ruler ({desc_ruler.value}) is Retrograde: The Astrologer's judgment may be impaired, delayed, or reversed.")
        if sun and desc_ruler != PlanetName.SUN:
            dist = abs(desc_planet.longitude - sun.longitude) % 360
            if dist > 180: dist = 360 - dist
            if (17/60) < dist <= 8.5:
                strictures.append(f"7th House Ruler ({desc_ruler.value}) is Combust: The Astrologer is mathematically blinded or incapable of seeing the entire truth of the matter.")
...
def check_besiegement(p: Planet, chart: Chart) -> Optional[Dict]:
    ...
    traditional_planets = [pl for pl in chart.planets if pl.name in {PlanetName.SUN, PlanetName.MOON, PlanetName.MERCURY, PlanetName.VENUS, PlanetName.MARS, PlanetName.JUPITER, PlanetName.SATURN}]
    
    sorted_planets = sorted(traditional_planets, key=lambda x: (x.longitude - p.longitude) % 360)
    ...
    if len(sorted_planets) >= 3:
        ...
        if next_planet.name in malefics and prev_planet.name in malefics and p.name not in malefics:
        ...
        if next_planet.name in benefics and prev_planet.name in benefics and p.name not in benefics:
            if next_planet.name != prev_planet.name:
                return {
...
def analyze_horary_physics(p1_name: PlanetName, p2_name: PlanetName, chart: Chart) -> List[Dict]:
    ...
    if besiegement_p1: conditions.append(besiegement_p1)
    
    besiegement_p2 = check_besiegement(p2, chart)
    if besiegement_p2: conditions.append(besiegement_p2)
    ...
    if p1.speed < 0 and p1.name not in {PlanetName.SUN, PlanetName.MOON}: # Sun/Moon never retrograde
        conditions.append({
...
KEYWORD_HOUSES = [
    ...
    (4, "Home/Property", ["house", "home", "property", "real estate", "apartment", "land", "move", "parent", "father", "mother"]),
    (7, "Relationships/Contracts", ["relationship", "marriage", "partner", "spouse", "boyfriend", "girlfriend", "dating", "divorce", "contract", "lawsuit", "opponent", "enemy"]),
    (2, "Money/Resources", ["money", "finance", "loan", "debt", "salary", "pay", "wealth", "income", "purchase", "buy", "sell", "investment"]),
    (6, "Health/Service", ["health", "illness", "disease", "medical", "surgery", "diagnosis", "workout", "pet", "dog", "cat", "employee"]),
    (5, "Children/Creation", ["child", "children", "pregnant", "baby", "fertility", "creative", "art", "romance"]),
    (9, "Travel/Study", ["travel", "visa", "immigration", "study", "school", "college", "university", "publishing", "foreign", "religion"]),
    (3, "Communication/Siblings", ["sibling", "brother", "sister", "neighbor", "message", "letter", "email", "communication", "trip", "car", "drive"]),
    (8, "Death/Other's Money", ["death", "taxes", "inheritance", "mortgage", "partner's money", "debt relief", "tax", "fear"]),
    (11, "Friends/Hopes", ["friend", "friends", "hope", "wish", "group", "club", "advisor", "network"]),
    (12, "Secrets/Undoing", ["secret", "prison", "jail", "magic", "hidden", "isolation", "hospital", "asylum", "witchcraft"]),
    (1, "Self/Life", ["myself", "me", "my life", "happiness", "my body", "appearance", "vitality"])
]
...
def evaluate_horary_conditions(conditions: List[Dict], condition_score: int, strength_score: int, strictures: List[str] = None) -> Dict[str, str | int]:
    ...
    perfection_events = {"Direct Application", "Easy Perfection", "Hard Perfection", "Regretful Perfection", "Translation of Light", "Collection of Light", "Antiscia", "Mutual Reception", "Bodily Placement"}
...
def build_horary_oracle(question: str, chart: Chart) -> Dict:
    ...
    verdict_data = evaluate_horary_conditions(conditions, condition_score["total_score"], strength_total, strictures)
