...
def calculate_firdaria(sect: Sect, birth_date: datetime, target_date: datetime) -> dict:
    ...
    if major_period in [PlanetName.NORTH_NODE, PlanetName.SOUTH_NODE]:
        ...
        if sub_idx > 6: sub_idx = 6
        
        # Sub-period sequence: starts with major ruler, follows the 7-planet sequence
        planets_only = [p for p, d in sequence if p not in [PlanetName.NORTH_NODE, PlanetName.SOUTH_NODE]]
...
def calculate_solar_return_jd(natal_sun_lon: float, birth_jd: float, target_year: int) -> float:
    """
...
class AdvancedPredictionEngine:
    ...
    """
    def __init__(self, natal_chart: Chart, birth_date: datetime, birth_jd: float, lat: float, lon: float):
