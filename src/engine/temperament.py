...
class TemperamentEngine:
    ...
    def calculate_temperament(chart: Chart) -> dict:
        ...
        if asc_ruler:
            ...
            details.append(f"Asc Ruler ({asc_ruler_name.value}) in {asc_ruler.sign.value}: +{ruler_qual}")
        ...
        phase_name, phase_qual = TemperamentEngine._get_moon_phase_qualities(phase_angle)
        ...
        primary = "Balanced"
        if net_hot > 0 and net_moist > 0: primary = "Sanguine (Hot/Moist)"
        elif net_hot > 0 and net_moist < 0: primary = "Choleric (Hot/Dry)"
        elif net_hot < 0 and net_moist < 0: primary = "Melancholic (Cold/Dry)"
        elif net_hot < 0 and net_moist > 0: primary = "Phlegmatic (Cold/Moist)"
    ...
    def _get_moon_phase_qualities(angle):
        if 0 <= angle < 90: return "New-1stQ", TemperamentEngine.MOON_PHASES["New to First Quarter"]
        elif 90 <= angle < 180: return "1stQ-Full", TemperamentEngine.MOON_PHASES["First Quarter to Full"]
        elif 180 <= angle < 270: return "Full-LastQ", TemperamentEngine.MOON_PHASES["Full to Last Quarter"]
        else: return "LastQ-New", TemperamentEngine.MOON_PHASES["Last Quarter to New"]

    @staticmethod
    def _get_season_qualities(sun_sign: Sign):
        # Seasons by Tropical Zodiac
        if sun_sign in [Sign.ARIES, Sign.TAURUS, Sign.GEMINI]: return "Spring", TemperamentEngine.SEASONS["Spring"]
        if sun_sign in [Sign.CANCER, Sign.LEO, Sign.VIRGO]: return "Summer", TemperamentEngine.SEASONS["Summer"]
        if sun_sign in [Sign.LIBRA, Sign.SCORPIO, Sign.SAGITTARIUS]: return "Autumn", TemperamentEngine.SEASONS["Autumn"]
