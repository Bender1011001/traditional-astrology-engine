...
def _send_report_email(customer_email: str, customer_name: str, pdf_bytes: bytes, chart_data: dict) -> None:
...
async def generate_premium_report_task(task_id: str, request_data: dict):
    ...
    try:
        ...
        report_markdown = await loop.run_in_executor(
            None, lambda: PremiumGenerator.generate_premium_report_markdown(chart_data)
        )
        ...
        if customer_email:
            try:
                ...
                pdf_buffer = await loop.run_in_executor(
                    None,
                    lambda: generator.generate(custom_content=report_markdown)
                )
                pdf_bytes = pdf_buffer.getvalue()
                customer_name = request_data.get("name", "Guest")
                await loop.run_in_executor(
                    None,
                    lambda: _send_report_email(customer_email, customer_name, pdf_bytes, chart_data)
                )
            except Exception as email_err:
                logger.error("PDF/email delivery failed for task %s: %s", task_id, repr(email_err), exc_info=True)
