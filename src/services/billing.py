...
class BillingService:
    ...
    def create_checkout_session(
        db: Session, user: User, plan_id: str, success_url: str, cancel_url: str
    ):
        # Find plan
        # We assume plan_id is the database ID or tier name?
        # Let's assume tier name since that's what frontend sends ("practitioner").
        plan = (
            db.query(SubscriptionPlan).filter(SubscriptionPlan.tier == plan_id).first()
        )
        if not plan:
            # Maybe it is a UUID
            plan = (
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            line_items=[
                {
                    "price": plan.stripe_price_id_monthly,  # type: ignore
                    "quantity": 1,
                }
            ],
            mode="subscription",
            ...
            metadata={"user_id": user.id, "plan_tier": plan.tier},  # type: ignore
        )
    ...
    def process_subscription_success(db: Session, session: dict):
        ...
        plan = (
            db.query(SubscriptionPlan)
            .filter(SubscriptionPlan.tier == plan_tier)
            .first()
        )
        ...
        sub.current_period_end = datetime.now(timezone.utc) + timedelta(
            days=30
        )  # Approximate, webhook should update
