...
class AstroAgent:
    ...
    def map_to_planet_name(k: str) -> PlanetName:
        k = k.upper()
        if k == "NORTH_NODE":
            return PlanetName.NORTH_NODE
        if k == "SOUTH_NODE":
            return PlanetName.SOUTH_NODE
        try:
            return PlanetName[k]
        except KeyError:
    ...
    def calculate(
        self, date_str: str, time_str: str, city: str, state: str = ""
    ) -> dict:
    ...
    def audit(self, chart_data: dict) -> dict:
        ...
        age = (
            analysis_dt.year
            - birth_dt.year
            - ((analysis_dt.month, analysis_dt.day) < (birth_dt.month, birth_dt.day))
        )
        ...
        audit_data = Auditor.perform_audit(
            chart, chart.jd or 0.0, birth_dt=birth_dt, ans_date=analysis_dt, age=age
        )

        technical_data = {
            "analysis": audit_data["analysis"],
            "planets_forensic": audit_data["planets_forensic"],
            "astronomy": {"houses": chart.houses or {}},
        }
    ...
    def run_full_dossier(
        ...
        return {"calculation": calc_data, "audit": audit_data}


def main():
    parser = argparse.ArgumentParser(
        description="AstroAgent: Internal Astrology Engine Interface for Agents"
    )
    parser.add_argument(
        "--action", choices=["calculate", "audit", "oracle", "full"], required=True
    )
