...
class MarketingAgent:
    ...
    def get_current_transits(self) -> dict:
        ...
        data = self.astro_agent.calculate(
            now.strftime("%Y-%m-%d"), now.strftime("%H:%M"), "New York", "NY"
        )
        return data
    ...
    def generate_hook(self, topic: str) -> str:
        ...
        return hooks.get(
            topic.lower(), f"Unlock the ancient wisdom of your chart with {topic}."
        )

    def generate_thread(self, topic: str, chart_context: dict = None) -> list[str]:  # type: ignore
        """
